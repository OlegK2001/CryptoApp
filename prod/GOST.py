...
from Bin_key import key

class Cripto:
    def __init__(self):
        # Ключи и компоненты шифрования
        self.KEY = key
        self.Test_key_1 = []
        self.Test_key_2 = []
        self.Test_key_3 = []
    ...
    def key_dev(self):
        ...
        self.imitation_insert = int(self.KEY[128*12+1:], 2)

        # Развертывание ключа
        self.expand_key(self.Test_key_1, self.Test_key_2)
    ...
    def go_to_encrtipto_message(self, message):
        self.message = message.encode('utf-8').hex()
        # Шифрование
        encrypted_blocks = [
    ...
    def go_to_decrtipto_message(self, encripto_message):
        # Расшифрование
        decrypted_blocks = [self.decrypt(block) for block in encripto_message]
        open_text = [self.bin_to_hex(block) for block in decrypted_blocks]
        return self.hex_string_to_text(open_text)

    def main(self) -> None:
        c = self.go_to_encrtipto_message("Привет")
        d = self.go_to_decrtipto_message(c)
