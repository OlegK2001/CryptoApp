...
class Cripto:
    ...
    def apply_sbox(self, input_block: NDArray[np.uint8]) -> NDArray[np.uint8]:
        ...
        """
        if not np.all((input_block >= 0) & (input_block < 256)): raise ValueError("Все значения входного массива должны быть в диапазоне от 0 до 255.")
    ...
    def galois_field_multiply(self, first: int, second: int) -> int:
        ...
        """
        if not (0 <= first < 256) or not (0 <= second < 256): raise ValueError("Оба аргумента должны быть в диапазоне от 0 до 255.")
        result = 0
        multiplicand = first
        multiplier = second
        for _ in range(self.BITS_IN_BYTE): # Умножение в поле Галуа методом "сдвиг-сложение"
            ...
            if high_bit: multiplicand ^= self.GALOIS_FIELD_MODULUS # Если старший бит был 1, выполняем редукцию по модулю
            multiplier >>= 1 # Сдвигаем множитель вправо
        return result
    ...
    def create_galois_multiplication_table(self) -> NDArray[np.uint8]:
        ...
        for i in range(256):
            for j in range(256): table[i, j] = self.galois_field_multiply(i, j)
        return table

    def start(self): # Предварительно вычисленная таблица умножения
    ...
    def apply_linear_transformation(self, input_block: NDArray[np.uint8]) -> NDArray[np.uint8]:
        ...
        """
        if input_block.shape != (self.BLOCK_SIZE,): raise ValueError("Входной массив состояния должен содержать ровно BLOCK_SIZE байтов.")
        a_15 = 0
    ...
    def apply_linear_function(self, input_block: NDArray[np.uint8]) -> NDArray[np.uint8]:
        ...
        for _ in range(self.BLOCK_SIZE): output_block = self.apply_linear_transformation(output_block)
        return output_block
    ...
    def apply_inverse_R_function(self, input_block: NDArray[np.uint8]) -> NDArray[np.uint8]:
        ...
        :raises ValueError: Если входной массив не имеет размерности BLOCK_SIZE.
        """
        if input_block.shape != (self.BLOCK_SIZE,): raise ValueError(f"Входной массив состояния должен содержать ровно {self.BLOCK_SIZE} байтов.")
    ...
    def apply_inverse_linear_function(self, input_block: NDArray[np.uint8]) -> NDArray[np.uint8]:
        ...
        if input_block.shape != (self.BLOCK_SIZE,): raise ValueError(f"Входной массив состояния должен содержать ровно {self.BLOCK_SIZE} байтов.")
        output_block = input_block
        for _ in range(self.BLOCK_SIZE): output_block = self.apply_inverse_R_function(output_block)
        return output_block
    ...
    def compute_round_constants(self) -> None:
        ...
        for i in range(self.ROUND_CONSTANTS_COUNT): self.iteration_constants[i] = self.apply_linear_function(blocks[i])

    def apply_feistel_round( self, key_part_1: NDArray[np.uint8], key_part_2: NDArray[np.uint8], iteration_constant: NDArray[np.uint8] ) -> tuple[NDArray[np.uint8], NDArray[np.uint8]]:
        """
        ...
        for arg in [key_part_1, key_part_2, iteration_constant]:
            if arg.shape != (self.BLOCK_SIZE,): raise ValueError(f"Входной массив должен содержать ровно {self.BLOCK_SIZE} байтов, получено: {arg.shape}.")
    ...
    def expand_key(self, master_key_1: NDArray[np.uint8], master_key_2: NDArray[np.uint8]) -> None:
        ...
        for arg, name in zip([master_key_1, master_key_2], ["master_key_1", "master_key_2"]):
            if arg.shape != (self.BLOCK_SIZE,): raise ValueError(f"{name} должен содержать ровно {self.BLOCK_SIZE} байтов, получено: {arg.shape}.")
        self.compute_round_constants()
        # Инициализация раундовых ключей
        self.round_keys[0] = master_key_1
        self.round_keys[1] = master_key_2
        temp_keys_1 = [master_key_1, master_key_2] # Генерация ключей
    ...
    def encrypt(self, input_block: NDArray[np.uint8]) -> NDArray[np.uint8]:
        ...
        """
        if input_block.shape != (self.BLOCK_SIZE,): raise ValueError(f"Входной блок должен содержать ровно {self.BLOCK_SIZE} байтов.")
        output_block = input_block
        ...
        return self.xor_arrays(output_block, self.round_keys[self.ACTUAL_ROUNDS])
    ...
    def hex_to_bin(self, hex_string: str) -> NDArray[np.uint8]:
        ...
        """
        if hex_string is None or hex_string == "":  return np.array([], dtype=self.ValueType)
    ...
    def bin_to_hex(self, bin_array: NDArray[np.uint8]) -> str:
        ...
        """
        if bin_array is None or len(bin_array) == 0:    return ""
        return ''.join([f'{x:02x}' for x in bin_array.astype(int)])

    def hex_string_to_text(self, hex_string): return bytes.fromhex(hex_string).decode('utf-8')

    def block(self, binary_string: str) -> str:
        if len(binary_string) % 32 != 0: binary_string = binary_string.zfill((len(binary_string) // 32 + 1) * 32) # Дополнить строку до длины, кратной 8
        return [binary_string[i:i + 32] for i in range(0, len(binary_string), 32)] # Разбить на блоки по 16 байт

    def main(self) -> None:
        TEST_KEY_1 = np.array([ 230, 198, 239, 184, 137, 245, 202,  85, 214,  70, 169,  23,   3, 199,  16, 181 ], dtype=self.ValueType)
        TEST_KEY_2 = np.array([ 167, 220, 154, 220,  54, 104,  81, 182, 132, 159,  27, 110, 207, 184,  42, 235 ], dtype=self.ValueType)
        self.message = "Привет? Jone&1234567890 wfw".encode('utf-8').hex()
        init_blocks = [self.hex_to_bin(iter) for iter in self.block(self.message)]
        self.expand_key(TEST_KEY_1, TEST_KEY_2) # Развертывание ключа
        encrypted_blocks = [ self.encrypt(self.hex_to_bin(block)) for block in self.block(self.message) ] # Шифрование
        decrypted_blocks = [self.decrypt(block) for block in encrypted_blocks] # Расшифрование
        open_text = [self.bin_to_hex(block) for block in decrypted_blocks]
        for block in open_text:
            print(self.hex_string_to_text(block))
