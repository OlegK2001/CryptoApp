...
class Cripto:
    ...
    def hex_string_to_text(self, hex_string):
    ...
    def block(self, binary_string: str) -> str:
        ...
        byte_blocks = [binary_string[i:i + 32] for i in range(0, len(binary_string), 32)]
        print('block: ', byte_blocks, len(byte_blocks))
        return byte_blocks
    ...
    def main(self) -> None:
        ...
        self.message = "Привет?1ауцадкаоцдлжуаоуцдауджауцдлалдьлдуцацуа1 ".encode('utf-8').hex()
        print('m:',  self.message)
        # Преобразование входных данных
        print('b: ', self.block(self.message))
        init_blocks = [self.hex_to_bin(iter) for iter in self.block(self.message)]

        print("init_blocks: ", init_blocks)
        # Развертывание ключа
        self.expand_key(TEST_KEY_1, TEST_KEY_2)

        for block in self.block(self.message):
            print("block for encrypt: ", block, ' -> ', self.hex_to_bin(block), end=' -> ')
